    ...
    */
            ...
            int CheckedWorkModePosition { get; set; } = 0;
            ...
            private void Form1_Load(object sender, EventArgs e)
            {
                ...
                dataGridView2.Columns[0].Name = "Класс";
                dataGridView2.Columns[1].Name = "Вертикаль";
                dataGridView2.Columns[2].Name = "Горизонталь";
    ...
    */
                    ...
                    foreach (var item in dbContext.Standart.ToList())
                    {
                        dataGridView1.Rows.Add($"{item.Letter}", $"{item.Horizontal}", $"{item.Vertical}", $"{FindEuclidian(zond.ZondCross.VerticalCrosses, item.Vertical, zond.ZondCross.HorisontalCrosses, item.Horizontal)}");//
                    }
                        dataGridView1.Rows.Add($"---", $"---", $"---", $"---");//
                    foreach (var item in dbContext.Image.ToList())
                    {
                        dataGridView2.Rows.Add($"{item.Letter}", $"{item.Horizontal}", $"{item.Vertical}");//
                    }
